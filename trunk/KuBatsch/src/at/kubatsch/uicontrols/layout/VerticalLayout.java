 ...
 *   filename: VerticalLayout.java
 *   project: KuBatsch
 */
package at.kubatsch.uicontrols.layout;
...
public class VerticalLayout implements LayoutManager
{
    ...
    private int             _vgap;     // the vertical vgap between
                                        // components...defaults to 5
    private int             _alignment; // LEFT, RIGHT, CENTER or BOTH...how the
                                        // components are justified
    private int             _anchor;   // TOP, BOTTOM or CENTER ...where are
                                        // the components positioned in an
                                        // overlarge space
    ...
    private Dimension layoutSize(Container parent, boolean minimum)
    {
        ...
        dim.height += insets.top + insets.bottom + _vgap + _vgap;
        return dim;
    }
    ...
    public void layoutContainer(Container parent)
    {
        ...
        synchronized (parent.getTreeLock())
        {
            ...
            y -= _vgap; // otherwise there's a vgap too many
            // Work out the anchor paint
            if (_anchor == TOP)
                y = insets.top;
            else if (_anchor == CENTER)
                y = (pd.height - y) / 2;
            else
    ...
    {
        return getClass().getName() + "[vgap=" + _vgap + " align=" + _alignment
                + " anchor=" + _anchor + "]";
    }
}
