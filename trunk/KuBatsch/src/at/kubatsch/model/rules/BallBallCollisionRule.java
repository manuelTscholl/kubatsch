...
public class BallBallCollisionRule extends BallRule
{
    ...
    protected boolean apply(Ball toApply, ICollidable collidesWith)
    {
        ...
        if (_appliedBalls.contains(toApply))
            return false;
        ...
        if (collidesWith instanceof Ball) // collides with another ball?
        {
            ...
            float v1 = (float) Math
                    .sqrt((toApply.getVelocity().x * toApply.getVelocity().x)
            ...
            Point2D.Float entity1 = new Point2D.Float(toApply.getVelocity().x / v1,
                    toApply.getVelocity().y / v1);
            Point2D.Float entity2 = new Point2D.Float(otherBall.getVelocity().x / v2,
                    otherBall.getVelocity().y / v2);
